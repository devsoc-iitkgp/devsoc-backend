...
const signupBody = zod.object({
  ...
  mobile: zod.string().regex(/^\d{10}$/, "Mobile must be a 10-digit number"), // Assuming mobile number is 10 digits
  email: zod
    .string()
...
const projectSchema = zod.object({
  ...
  thumbnail: zod.string().url("Thumbnail must be a valid URL"),
});
