...
const signup = async (req: Request, res: Response) => {
  try {
    ...
    if (!parseResult.success) {
      return res
        .status(400)
        .json({ message: "Invalid Inputs", success: false });
    }
    const {
      name,
      username,
...
const fetchUserDetails = async (req: Request, res: Response) => {
  ...
  if (!user) {
    return res
      .status(400)
      .json({ message: "User data is not available", success: false });
  }
  try {
    return res.status(200).json({
      user,
...
const userUpdate = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (parseResult.data.password) {
      ...
      const saltRounds = 10;
      parseResult.data.password = await bcrypt.hash(
...
const sendRequest = async (req: Request, res: Response) => {
  try {
    ...
    const projectRequested = await prismaClient.user.update({
      where: { id: userId },
      data: {
        requests: {
